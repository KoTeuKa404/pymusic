...
try:
    ...
    WindowManagerLayoutParams = None
...
def _install_crash_hooks():
    ...
    try:
        ...
        _faulthandler_file = open(LOG_PATH, "a", buffering=1, encoding="utf-8")
        faulthandler.enable(file=_faulthandler_file, all_threads=True)
        log("[CRASH] faulthandler enabled")
    except Exception:
        pass
...
@run_on_ui_thread
def apply_light_system_bars():
    """Робить системний status/navigation bar світлим, щоб кнопки Android були видимі."""
    try:
        ...
        try:
            if WindowManagerLayoutParams is not None:
                window.clearFlags(
                    WindowManagerLayoutParams.FLAG_TRANSLUCENT_STATUS
                    | WindowManagerLayoutParams.FLAG_TRANSLUCENT_NAVIGATION
                )
        ...
        try:
            ...
            flags = int(decor.getSystemUiVisibility() or 0)

            # Не ховаємо системні кнопки. Нам потрібен звичайний Android navigation bar.
            for name in (
                ...
                try:
                    flags &= ~int(getattr(View, name))
                except Exception:
                    pass

            if Build_VERSION.SDK_INT >= 23:
                flags |= int(getattr(View, "SYSTEM_UI_FLAG_LIGHT_STATUS_BAR", 8192))
            if Build_VERSION.SDK_INT >= 26:
                flags |= int(getattr(View, "SYSTEM_UI_FLAG_LIGHT_NAVIGATION_BAR", 16))
        ...
        try:
            log("[UI] light system bars applied")
        except Exception:
            pass
    except Exception as e:
        try:
            log(f"[UI] light system bars err: {e}")
        except Exception:
            pass
...
def _is_youtube_playback_browse_url(url: str) -> bool:
    """True only for Music albums/playlists/mixes. Channel/author browse URLs must open in WebView."""
    if not url:
        return False
    try:
        ...
        if marker not in path:
            return False
        browse_id = path.split(marker, 1)[1].split("/", 1)[0].split("?", 1)[0]
        if not browse_id:
            return False
        if browse_id.startswith("UC") or browse_id.startswith("@"):
            return False
        return browse_id.startswith(("MPRE", "OLAK5uy", "VL", "PL", "RD"))
    except Exception:
        u = str(url).lower()
        return (
            any(x in u for x in ("/browse/mpre", "/browse/olak5uy", "/browse/vl", "/browse/pl", "/browse/rd"))
            and "/browse/uc" not in u
        )
...
def _is_youtube_watch_url(url: str) -> bool:
    ...
    return (
        ...
        or _is_youtube_playback_browse_url(url)
        or "youtu.be/" in u
...
def is_network_available():
    try:
        ...
        try:
            ...
            pass
        # Some Android/WebView builds do not mark the network as VALIDATED fast enough
        # during app startup. If the active network at least advertises INTERNET, allow YT;
        # pure offline mode still returns False when there is no active network at all.
        try:
...
ACTION_OPEN_PLAYER = None


def _ensure_action_constants():
    global _ACTION_PREFIX, ACTION_PREV, ACTION_PLAY, ACTION_PAUSE, ACTION_TOGGLE, ACTION_NEXT, ACTION_REPEAT, ACTION_SEEK, ACTION_WEB_URL, ACTION_WEB_MODE, ACTION_OPEN_PLAYER
    if _ACTION_PREFIX:
        return
    try:
    ...
    ACTION_OPEN_PLAYER = f"{_ACTION_PREFIX}.OPEN_PLAYER"
...
def create_or_update_media_notification(*,
    ...
    try:
        ...
        # manifest actually launches a subclass (MediaKeyActivity). Use the package
        # launch intent / current activity class instead.
        intent_open = None
        try:
            pm = act.getPackageManager()
            intent_open = pm.getLaunchIntentForPackage(act.getPackageName())
        except Exception as e:
            log(f"[NOTIF] launch intent err: {e}")
            intent_open = None
        if intent_open is None:
            try:
                intent_open = Intent(act, act.getClass())
            except Exception:
                intent_open = Intent(act, PythonActivity)
        try:
            intent_open.setAction(ACTION_OPEN_PLAYER)
        except Exception:
            pass
        try:
            intent_open.putExtra("open_player", True)
            intent_open.putExtra("from_notification", True)
            intent_open.putExtra("notification_id", int(NOTIF_ID))
        except Exception:
            pass
        try:
            intent_open.addFlags(
...
def _route_action_intent(intent):
    try:
        ...
        if act == ACTION_OPEN_PLAYER:
            handled = False
            try:
                owner = _action_owner
                cb = getattr(owner, "_open_player_from_notification", None) if owner is not None else None
                if callable(cb):
                    cb()
                    handled = True
            except Exception as e:
                log(f"[NOTIF] open player cb err: {e}")
            if not handled:
                try:
                    _dispatch_mode("audio")
                except Exception as e:
                    log(f"[NOTIF] open player fallback err: {e}")
            return
...
_last_consumed_intent_sig = None
...
def _intent_signature(intent):
    try:
        ...
        act = ""
    try:
        data = str(intent.getDataString() or "")
    except Exception:
        data = ""
    try:
        open_flag = bool(intent.getBooleanExtra("open_player", False))
    except Exception:
        open_flag = False
    try:
        notif_id = int(intent.getIntExtra("notification_id", -1))
    except Exception:
        notif_id = -1
    return (act, data, open_flag, notif_id)


def consume_current_activity_intent():
    """Handle notification tap even when Android resumes the existing Activity
    without delivering a Python on_new_intent callback.
    Safe to call from on_start/on_resume.
    """
    global _last_consumed_intent_sig
    try:
        _ensure_action_constants()
        act_obj = PythonActivity.mActivity
        intent = act_obj.getIntent()
        if intent is None:
            return False
        ...
        try:
            ...
            open_flag = False
        if action != ACTION_OPEN_PLAYER and not open_flag:
            return False
        sig = _intent_signature(intent)
        if sig == _last_consumed_intent_sig:
            return False
        _last_consumed_intent_sig = sig
        log(f"[NOTIF] consume current intent {sig}")
        _route_action_intent(intent)
        # Clear the stored launch intent so a normal resume later does not
        # repeatedly force the user back to the audio screen.
        try:
            clean = Intent()
            act_obj.setIntent(clean)
        except Exception:
            pass
        return True
    except Exception as e:
        log(f"[NOTIF] consume current intent err: {e}")
        return False
...
def _on_new_intent(intent):
    try:
        # Keep Activity.getIntent() in sync for on_resume fallback.
