    ...
    class _WebUiClient(PythonJavaClass):
        ...
        def onPageFinished(self, view, url):
            try:
                js = (
                    "try{"
                    "var css='#masthead, ytd-masthead, ytm-mobile-topbar-renderer, ytm-app-bar, ytm-pivot-bar-renderer,"
                    "#header, #header-bar, #appbar, #app-promo, #app-banner, .app-promo, .app-banner, .ytm-mobile-topbar-renderer"
                    ...
                    "s.textContent=css;"
                    "var css='#masthead, ytm-mobile-topbar-renderer, ytm-app-bar, ytm-pivot-bar-renderer,"
                    "#header, #header-bar, #appbar, #app-promo, #app-banner, .app-promo, .app-banner,"
                    ".ytm-mobile-topbar-renderer, ytm-video-with-context-renderer, ytm-watch-metadata,"
                    "ytm-item-section-renderer, ytm-compact-autoplay-renderer, ytm-chip-cloud-renderer"
                    "{display:none!important;}'"
                    "+'html,body{margin:0!important;padding:0!important;width:100%!important;height:100%!important;"
                    "overflow:hidden!important;background:#000!important;}'"
                    "+'#player, ytm-player, .player-container{position:fixed!important;left:0;top:0;"
                    ...
                    "if(!s){s=document.createElement('style');s.id='pymusic-hide';document.documentElement.appendChild(s);}"
                    "s.textContent=css;"
                    "var v=document.querySelector('video');if(v){v.muted=false;v.volume=1;}"
                    "var btn=document.querySelector('.ytp-mute-button, .ytp-unmute,"
                    "button[aria-label*=\"Unmute\"], button[aria-label*=\"Увімкнути звук\"],"
                    "button[aria-label*=\"Включить звук\"], button[aria-label*=\"Unmute\"]');"
                    "if(btn){btn.click();}"
                    "}catch(e){}"
                )
                if view is not None:
