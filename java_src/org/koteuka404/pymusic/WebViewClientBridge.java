...
public class WebViewClientBridge extends WebViewClient {
    ...
    private static boolean isPlaybackBrowseUrl(String url) {
        if (url == null) return false;
        try {
            Uri uri = Uri.parse(url);
            String path = uri.getPath();
            if (path == null) return false;
            int idx = path.indexOf("/browse/");
            if (idx < 0) return false;
            String id = path.substring(idx + 8);
            int slash = id.indexOf('/');
            if (slash >= 0) id = id.substring(0, slash);
            int q = id.indexOf('?');
            if (q >= 0) id = id.substring(0, q);
            if (id.isEmpty()) return false;
            // UC... та @... — це канали/автори. Їх у WEB треба відкривати, а не перехоплювати як трек.
            if (id.startsWith("UC") || id.startsWith("@")) return false;
            // MPRE/OLAK/VL/PL/RD — альбоми, плейлисти, мікси. Їх можна віддавати в аудіо-чергу.
            return id.startsWith("MPRE")
            ...
            return (u.contains("/browse/mpre")
                    || u.contains("/browse/olak5uy")
                    || u.contains("/browse/vl")
                    || u.contains("/browse/pl")
                    || u.contains("/browse/rd"))
                    && !u.contains("/browse/uc");
        }
    }
    ...
    private static boolean isYoutubeHost(String url) {
        ...
        try {
            ...
            String h = host.toLowerCase();
            return "youtube.com".equals(h)
                    || "www.youtube.com".equals(h)
                    || "m.youtube.com".equals(h)
                    || "music.youtube.com".equals(h)
                    || "youtu.be".equals(h);
        } catch (Exception ignored) {
            return false;
        }
    ...
    private static boolean isOfflineLikeError(int errorCode) {
    ...
    private void openSearchInsteadOfBrokenYoutube(WebView view, String failingUrl, int errorCode) {
        if (!isOfflineLikeError(errorCode)) return;
        if (!isYoutubeHost(failingUrl)) return;
        try {
            if (view != null) {
                view.stopLoading();
    ...
    private void injectHook(WebView view) {
        ...
        String js = "(function(){"
                ...
                + "function isPlaybackBrowse(u){try{var a=new URL(u,location.href);var m=(a.pathname||\'\').match(/\\/browse\\/([^\\/?#]+)/);if(!m)return false;var id=m[1]||\'\';if(/^UC/.test(id)||/^@/.test(id))return false;return /^(MPRE|OLAK5uy|VL|PL|RD)/.test(id);}catch(e){return /\\/browse\\/(MPRE|OLAK5uy|VL|PL|RD)/i.test(u)&&!/\\/browse\\/UC/i.test(u);}}"
                + "function isWatch(u){return /youtube\\.com\\/(watch|shorts|live|playlist)\\b/.test(u)||/m\\.youtube\\.com\\/(watch|shorts|live|playlist)\\b/.test(u)||/music\\.youtube\\.com\\/(watch|playlist)\\b/.test(u)||/youtu\\.be\\//.test(u)||isPlaybackBrowse(u);}"
                + "function ensureUi(){"
                + "try{"
    ...
    public void onReceivedError(WebView view, int errorCode, String description, String failingUrl) {
        ...
        // On Android 6+ the newer overload below tells us whether the error is for
        // the main frame. The old overload can also fire for subresources, and that
        // was forcing the app back to Web/search even when YT itself worked.
        if (Build.VERSION.SDK_INT >= 23) return;
        openSearchInsteadOfBrokenYoutube(view, failingUrl, errorCode);
    }
    ...
    public void onReceivedError(WebView view, WebResourceRequest request, WebResourceError error) {
        ...
        openSearchInsteadOfBrokenYoutube(view, failingUrl, error.getErrorCode());
    }
}
